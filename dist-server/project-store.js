...
export class ProjectStore {
    ...
    constructor(dataDir = DATA_DIR) {
        ...
        this.projects = readJson(this.projectsPath, []);
        ...
        this.assignments = readJson(this.assignmentsPath, []);
        this.workItems = readJson(this.workItemsPath, []);
    }
    save() {
    ...
    createProject(input) {
        ...
        if (!Object.hasOwn(DEFAULT_LANES, kind))
            throw new Error(`unsupported project kind '${kind}'`);
    ...
    assignBot(input) {
        ...
        if (!lane)
            throw new Error(`lane '${input.laneId}' does not exist in this project`);
        const existing = this.assignments.find((assignment) => assignment.projectId === project.id && assignment.botId === input.botId && assignment.laneId === lane.id);
        if (existing) {
    ...
    workItemsForProject(projectId, includeDone = false) {
    ...
    createWorkItem(input) {
        ...
        if (!title)
            throw new Error("work item title required");
        if (input.laneId && !project.lanes.some((lane) => lane.id === input.laneId))
            throw new Error("lane does not belong to project");
        const now = Date.now();
        const evidence = Array.isArray(input.evidence)
            ? input.evidence
                .filter((entry) => entry && typeof entry === "object" && typeof entry.kind === "string" && typeof entry.ref === "string")
                .slice(0, 25)
                .map((entry) => ({ kind: entry.kind, ref: entry.ref, ...(entry.label ? { label: String(entry.label) } : {}) }))
            : [];
        const item = {
            id: newId(),
            projectId: project.id,
    ...
    patchWorkItem(id, patch) {
        ...
        if (patch.laneId && project && !project.lanes.some((lane) => lane.id === patch.laneId))
            throw new Error("lane does not belong to project");
        ...
        for (const entry of context.projects) {
            ...
            for (const assignment of entry.assignments) {
                if (assignment.instructions)
                    lines.push(`${assignment.roleName} instructions: ${assignment.instructions}`);
            }
            if (entry.project.rootPath)
                lines.push(`Project root: ${entry.project.rootPath}`);
            if (entry.project.repositoryUrl)
                lines.push(`Repository: ${entry.project.repositoryUrl}`);
            if (entry.project.mcpTools.length)
                lines.push(`Project MCP bundles: ${entry.project.mcpTools.join(", ")}`);
            const items = entry.activeWorkItems.slice(0, 12);
            if (items.length) {
                lines.push("Open tracked work:");
                for (const item of items)
                    lines.push(`- [${item.priority}] ${item.kind}/${item.status}: ${item.title} (${item.id})`);
            }
        }
...
export const projectKinds = Object.keys(DEFAULT_LANES);
