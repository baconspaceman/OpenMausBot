...
function candidateServers(cfg, options) {
    ...
    return Object.entries(merged)
        ...
        if (server.enabled === false)
            return false;
        if (options?.toolNames?.length && !options.toolNames.includes(name))
            return false;
...
export async function discoverLocalMcp(cfg, options) {
    const candidates = candidateServers(cfg, options);
    const results = await Promise.all(candidates.map(async (server) => (server.command || (server.url && await reachable(server.url)) ? server : null)));
