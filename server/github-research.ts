...
function targetFromUrl(raw: string) {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
  ...
  if (parts.length < 2) throw new GitHubResearchError("The GitHub URL does not contain an owner and repository.");
  const owner = parts[0];
  const repo = parts[1].replace(/\.git$/, "");
  if (!/^[A-Za-z0-9_.-]+$/.test(owner) || !/^[A-Za-z0-9_.-]+$/.test(repo)) {
    throw new GitHubResearchError("The GitHub URL contains an unsupported repository name.");
  }
  if (parts[2] === "issues" && /^\d+$/.test(parts[3] ?? "")) return { kind: "issue" as const, owner, repo, number: parts[3] };
  if (parts[2] === "pull" && /^\d+$/.test(parts[3] ?? "")) return { kind: "pull" as const, owner, repo, number: parts[3] };
  if ((parts[2] === "blob" || parts[2] === "tree") && parts.length >= 5) {
    return { kind: parts[2] as "blob" | "tree", owner, repo, ref: parts[3], path: parts.slice(4).join("/") };
  }
  return { kind: "repo" as const, owner, repo };
}
...
async function githubApi(path: string) {
  ...
  if (!response.ok) {
    if (response.status === 404) throw new GitHubResearchError("That GitHub resource is private, missing, or unavailable anonymously.", 404);
...
export async function inspectGitHubUrl(raw: string) {
  ...
  if (target.kind === "issue" || target.kind === "pull") {
    ...
    const result: Record<string, unknown> = {
      ...
      labels: Array.isArray(item.labels) ? item.labels.slice(0, 30).map((label: any) => label.name) : [],
  ...
  if (target.kind === "tree" || content.type === "dir") {
    return {
      ...
      url: raw,
      path: target.path,
      entries: Array.isArray(content) ? content.slice(0, 100).map((entry: any) => ({ name: entry.name, type: entry.type, path: entry.path, url: entry.html_url })) : [],
    };
  }
  if (content.type !== "file" || typeof content.content !== "string") throw new GitHubResearchError("That GitHub link is not a readable file or directory.");
