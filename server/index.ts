...
import { discoverLocalMcp, PROJECT_MCP_CATALOG } from "./local-mcp.ts";
import {
...
let shutdownManagedComputersPromise: Promise<unknown> | null = null;
function shutdownManagedComputers() {
  if (shutdownManagedComputersPromise) return shutdownManagedComputersPromise;
  shutdownManagedComputersPromise = (async () => {
    ...
    const cloud = cfg.box?.token
      ...
      : [];
    for (const result of local) console.log(`[lifecycle] shutdown ${result.backend}: ${result.detail}`);
    if (cloud.length) console.log(`[lifecycle] requested sleep for ${cloud.length} Box computer(s)`);
    return { local, cloud };
  })();
...
function cuaConnectionCandidates(): string[] {
  ...
  if (explicit) return [join(explicit, "cua-connection.json")];
  const roots =
...
async function startTurn(botId: string, text: string) {
  ...
  const projectCwd = projectContext.projects
    ...
    .find((rootPath): rootPath is string => Boolean(rootPath && existsSync(rootPath)));
  ...
  void (async () => {
    try {
      ...
      if (isShellBackend(wants)) {
        ...
        integrations.computer = { backend: wants, config: backendConfig(cfg, wants) as Record<string, unknown> };
      ...
      if (integrations.computer && "boxId" in integrations.computer) startScreenPoller(bot.id);
    } catch (e) {
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (method === "GET" && path === "/api/projects/context") {
      ...
      if (!botId) return json(res, 400, { error: "bot_id required" });
    ...
    if (m && method === "GET") {
      ...
      if (!context) return json(res, 404, { error: "no such project" });
    ...
    if (m && method === "POST") {
      ...
      if (!project) return json(res, 404, { error: "no such project" });
      const body = await readBody(req);
      if (!store.bot(String(body.botId ?? ""))) return json(res, 404, { error: "no such bot" });
    ...
    if (m && method === "PATCH") {
      ...
      if (!assignment) return json(res, 404, { error: "no such assignment" });
      broadcast({ kind: "project.assignment", assignment });
    ...
    if (m && method === "PATCH") {
      ...
      if (!existingItem) return json(res, 404, { error: "no such work item" });
      const botId = String(body.botId ?? "").trim();
      ...
      if (!workItem) return json(res, 404, { error: "no such work item" });
      broadcast({ kind: "project.work-item", workItem });
    ...
    if (method === "POST" && path === "/api/lifecycle/shutdown") {
      return json(res, 200, { ok: true, ...(await shutdownManagedComputers()) as object });
    }
    ...
    if (m && method === "GET") {
      const bot = store.bot(m[1]);
      if (!bot) return json(res, 404, { error: "no such bot" });
      if (isShellBackend(bot.computer)) return json(res, 200, await backendStatus(cfg, bot.computer));
